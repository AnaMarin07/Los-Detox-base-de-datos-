...
<head>
    <meta charset="UTF-8">
    <title>Detox Project - Home</title>
    <link rel="stylesheet" href="PHP/CSS/Pagina principal.css">
    <style>
        /* Estilos extra para que las cards se vean de 10 */
        .card-producto:hover {
...
<body>
    ...
    <nav class="navbar">
        ...
        <div class="nav-icons">
            ...
            <div style="position: relative; display: inline-block;">
                <svg onclick="toggleCarrito()" class="icon" viewBox="0 0 24 24" width="24" height="24" stroke="black" stroke-width="2" fill="none" style="cursor: pointer;"><circle cx="9" cy="21" r="1"></circle><circle cx="20" cy="21" r="1"></circle><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path></svg>
                <?php if (!empty($_SESSION['carrito'])): ?>
                    <span style="position: absolute; top: -8px; right: -8px; background: red; color: white; border-radius: 50%; width: 16px; height: 16px; font-size: 10px; display: flex; align-items: center; justify-content: center;"><?php echo count($_SESSION['carrito']); ?></span>
                <?php endif; ?>
                <div id="carrito_visual" style="display: none; position: absolute; right: 0; top: 35px; width: 300px; background: white; border: 1px solid #eee; box-shadow: 0 4px 10px rgba(0,0,0,0.1); z-index: 1000; padding: 15px;">
                    <?php if (empty($_SESSION['carrito'])): ?>
                        <p style="color: #888; font-size: 13px;">¡Tu carrito está vacío!</p>
                    <?php else: ?>
                        ...
                        <?php endforeach; ?>
                        <p style="text-align: right; font-weight: bold; margin-top: 10px;">TOTAL: $<?php echo $total; ?></p>
                        <a href="PHP/carrito.php"><button style="width: 100%; background: black; color: white; border: none; padding: 10px; cursor: pointer; font-weight: bold; margin-top: 10px;">VER CARRITO</button></a>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <div id="barrita_de_busqueda" style="display: none; background: white; padding: 20px 40px; border-bottom: 1px solid #ddd;">
        <form action="PHP/buscar.php" method="GET" style="max-width: 1000px; margin: 0 auto; display: flex; align-items: center; gap: 15px;">
            <input type="text" name="query" placeholder="Buscar productos..." style="width: 100%; border: none; font-size: 20px; outline: none;" required>
    ...
    <section class="main-banner">
        <div class="banner-content">
            ...
            <h1>LIBERA TU ENERGÍA</h1>
            <a href="PHP/Productos.php"><button class="btn-banner">PANEL PRODUCTOS</button></a>      <a href="PHP/ver_clientes.php"><button class="btn-banner">PANEL CLIENTES</button></a>
        </div>
    </section>

    <div style="padding: 60px 20px; text-align: center; background-color: #fcfcfc;">
        <h1 style="font-size: 45px; font-weight: 900; letter-spacing: -2px; margin-bottom: 10px;">PRODUCTOS DESTACADOS</h1>
        <p style="color: #888; margin-bottom: 40px;">Lo más nuevo de la temporada</p>
        
        <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 30px;">
            <?php 
            include('PHP/basededatos.php'); 
            // Query uniendo con la tabla 'stock' y agrupando talles
    ...
    <script>
        ...
        function toggleCarrito() {
            var c = document.getElementById("carrito_visual");
